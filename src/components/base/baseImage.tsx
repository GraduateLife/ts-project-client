'use client';
import React, { ImgHTMLAttributes, useState } from 'react';
import Image from 'next/image';
import { cn } from '@/lib/utils';
import { genNumber } from '@/mock/constants';

type ImageProps = {
  src: string;
  alt?: string;
} & ImgHTMLAttributes<HTMLImageElement>;

const placeholderImageBase64Code =
  '' as const;

const BaseImage = ({
  src,
  alt = genNumber(1, 5000).toString(),
  className,
}: ImageProps) => {
  const [requestUrl, setRequestUrl] = useState(src);
  const [onErrorClassName, setOnErrorClassName] = useState('');

  const handleloadError = () => {
    setOnErrorClassName('object-contain');
    setRequestUrl(`/error.png`);
    return;
  };

  return (
    <Image
      src={requestUrl}
      alt={alt}
      // fill
      width={800}
      height={600}
      className={cn('object-cover', className, onErrorClassName)}
      loading="lazy"
      placeholder={`data:image/png;base64,${placeholderImageBase64Code}`}
      onError={handleloadError}
    />
  );
};

export default BaseImage;
